...
class GraphTransformerNetwork(nn.Module):
	def __init__(self, 
		...
		# Initialize the DGLGraphTransformer
		self.dgl_transformer = DGLGraphTransformer(in_channels=in_feats,
													edge_features=in_edge_feats,
													num_hidden_channels=hidden_size,
													num_layers=num_layers)
		self.output_dim = hidden_size

	def forward(self, g, features):
		# Unpack features into node and edge features for both graphs
		node_feats1, edge_feats1 = features['node'], features['edge']
		
		# Apply the DGLGraphTransformer to both graphs (edge features will be encoded inside)
		h = self.dgl_transformer(g, node_feats1, edge_feats1)
		g.ndata['atom'] = h
		# Perform mean pooling on the node features from both graphs
		mean_h = dgl.mean_nodes(g, 'atom')
		return mean_h
